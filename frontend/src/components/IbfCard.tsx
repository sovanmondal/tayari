  ...
  return (
    <div className="rounded-xl bg-slate-800/80 p-4">
      <div className="flex items-center justify-between">
        ...
        <span
          className="rounded-full px-3 py-1 text-sm font-semibold text-slate-900"
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <div className="rounded-lg bg-slate-900/60 p-3">
          ...
          <div className="text-xs text-slate-400">people exposed</div>
        </div>
        <div className="rounded-lg bg-slate-900/60 p-3">
            ...
            {ibf.impact.by_livelihood.length
              ? ibf.impact.by_livelihood.map((b) => (
                  <div key={b.livelihood} className="flex justify-between">
                    <span className="text-slate-400">{b.livelihood}</span>
                    <span>{num(b.population)}</span>
                  </div>
                ))
              : <span className="text-slate-500">Not triggered</span>}
          </div>
        </div>
      </div>

      <p className="mt-3 text-sm leading-relaxed text-slate-200">{ibf.narrative}</p>
